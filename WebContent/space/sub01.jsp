...
/*
파라미터를 저장할 용도로 생성한 Map컬렉션. 여러개의 파라미터는 한꺼번에
저장한 후 DAO의 메소드를 호출할때 전달할것임.
차후 프로그램의 업데이트에 의해 파라미터가 추가되더라도 Map을 사용하므로 메소드의
변형없이 사용할 수 있다.
*/
...
if(searchWord!=null){
	/*
	리스트 페이지에 최초 진입시에는 파라미터가 없으므로 if로 구분하여
...
//board테이블에 입력된 전체레코드 갯수를 카운트하여 반환
...
/*  
전체페이지수 계산 => 게시물이 108개라 가정하면 108/10 => 10.8 => ceil(10.8)=>11페이지
*/
int totalPage = (int)Math.ceil((double)totalRecordCount/pageSize);

/*  
현재페이지번호 : 파라미터가 없을때는 무조건 1페이지로 저장하고, 값이 있을 때는 해당값을 얻어와서 숫자로 변경한다.
...
// MariaDB에서는 limit를 사용하므로 계산식이 조금 달라지게 된다.
//page 1 2 3
//    0x2  1x2  2x2
//     0    2    4
//     2    2    2
...
List<BbsDTO> bbs = dao.selectListPageSearch(param);//페이지처리ㅇ+회원이름 검색
		...
		<!-- ### 게시판의 body 부분 start -->
			...
			<div class="row mt-3">
				 ...
				 */
				 for(BbsDTO dto : bbs){
					 /*  
					 전체 레코드수를 이용하여 가상번호를 부여하고 반복시 1씩 차감한다.
					 */
				 ...
				 <tr>
				 	...
				 	<td>
				 		<a href="BoardView.jsp?num=<%=dto.getNum()%>&nowPage=<%=nowPage %>&<%=queryStr%>"><%=dto.getTitle() %>
				 		</a>
				 	</td>
